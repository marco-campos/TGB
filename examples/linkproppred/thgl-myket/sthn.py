...
for run_idx in range(NUM_RUNS):
    ...
    model, train_history, best_epoch, best_auc, best_valid_loss = link_pred_train(
    ...
    curve_filename = f'{loss_curves_path}/{MODEL_NAME}_{DATA}_seed{run_seed}_run{run_idx}.json'
    curve_payload = {
        'model': MODEL_NAME,
        'data': DATA,
        'run': run_idx,
        ...
        'valid_loss': [float(x) for x in train_history.get('valid_loss', [])],
        'train_auc': [float(x) for x in train_history.get('train_auc', [])],
        'valid_auc': [float(x) for x in train_history.get('valid_auc', [])],
    }
