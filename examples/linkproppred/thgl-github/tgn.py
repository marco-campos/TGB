...
NUM_NEIGHBORS = 10
USE_EDGE_TYPE = bool(args.use_edge_type)
USE_NODE_TYPE = bool(args.use_node_type)
...
loss_curves_path = f'{results_path}/loss_curves'
Path(loss_curves_path).mkdir(parents=True, exist_ok=True)
...
for run_idx in range(NUM_RUNS):
    ...
    for epoch in range(1, NUM_EPOCH + 1):
        ...
        train_loss_curve.append(float(loss))
        ...
        val_metric_curve.append(float(perf_metric_val))
    ...
    run_seed = run_idx + SEED
    curve_filename = (
        f"{loss_curves_path}/{MODEL_NAME}_{'edge_type' if USE_EDGE_TYPE else 'plain'}_"
        f"{DATA}_seed{run_seed}_run{run_idx}.json"
    )
    curve_payload = {
        'model': MODEL_NAME,
        'variant': 'tgn_edge_type' if USE_EDGE_TYPE else 'tgn',
        'data': DATA,
        'run': run_idx,
        'seed': run_seed,
        'best_epoch': best_epoch,
        f'best_val_{metric}': float(best_val),
        f'best_test_{metric}': float(best_test),
        'train_loss': train_loss_curve,
        f'val_{metric}': val_metric_curve,
    }
    with open(curve_filename, 'w') as fp:
        json.dump(curve_payload, fp, indent=4)
